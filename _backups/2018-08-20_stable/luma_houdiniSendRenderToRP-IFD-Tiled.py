...
def buildCmdLineIFD(l_job):
    ...
    cmd += ' -nj_renderer GenerateIFD/16.5.473_Tiled'
...
def buildCmdLineEXR(l_job):
    ...
    cmd += ' "-nj_name" "%s"' % (l_job['name'])
    cmd += ' "-nj_priority" "%d"' % (l_job['priority'])
    cmd += ' "-nj_renderer" "RenderEXR/16.5.473_Tiled"'
    cmd += ' "-nj_splitmode" "2,1"'
...
def main():
    ...
    for rop in rop_list:
        ...
        g_jobIFD['name']   = "IFD-GEN-TILED : %s - %s - %s" % (os.environ["USERNAME"], os.environ["HIPNAME"], rop.name())
        ...
        # g_jobEXR['name']   = "IFD-REN-TILED : %s - %s - %s" % (os.environ["USERNAME"], os.environ["HIPNAME"], rop.name())
        # files = ""
